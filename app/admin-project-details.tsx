...
const VolunteerCard = ({ volunteer, onApprove }: VolunteerCardProps) => (
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView style={styles.container}>
        ...
        {project.pendingVolunteers.length > 0 ? (
          project.pendingVolunteers.map(v => <VolunteerCard key={v.id} volunteer={v} onApprove={handleApprove} />)
        ) : (
        ...
        {project.approvedVolunteers.length > 0 ? (
          project.approvedVolunteers.map(v => <VolunteerCard key={v.id} volunteer={v} onApprove={null} />)
        ) : (
...
const styles = StyleSheet.create({
  ...
  volunteerCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: 15, borderRadius: 8, marginBottom: 10 },
  volunteerName: { fontSize: 16, fontWeight: '600' },
  volunteerInfo: { fontSize: 14, color: 'gray', marginTop: 4 },
  approveButton: { backgroundColor: '#28a745', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 5 },
